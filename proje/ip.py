...
class WorkerThread (threading.Thread):
   ...
   def grayscale(self, header, patch, threshold):

       newMessage = [0] * self.patchsize * self.patchsize

       for column in xrange(patch.getHeight()):
           for row in xrange(patch.getWidth()):
               r, g, b = patch.getPixel(row, column)
               r = int(r*0.299)
               g = int(g*0.587)
               b = int(b*0.114)
               lum = r+g+b
               patch.setPixel(row, column, (lum, lum, lum))
